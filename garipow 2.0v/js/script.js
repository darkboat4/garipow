...
var placar = document.querySelector('.placarNomes');
var containerAll = document.querySelector('.container-all')
var pplacar = document.querySelector('.pplacar')
var voltarLeaboard = document.querySelector('.voltar')
var addPlacar = document.querySelector('.add-placar')

document.body.removeChild(addPlacar)
document.body.removeChild(pplacar)

function btnOpen(){
    game.appendChild(pplacar);
    game.removeChild(start)
    game.removeChild(ladeboard)
}
function fecharLadeboard(){
    game.removeChild(pplacar);
    game.appendChild(start);
    game.appendChild(ladeboard);
}

var ladeboard = document.createElement('button');
    ladeboard.setAttribute("class", "ladeboard");
    ladeboard.addEventListener('click', function() {
        btnOpen();
    });
ladeboard.textContent = "leaderboard";
game.appendChild(ladeboard);
// function voltarLeaboard() {
//     fecharLadeboard();
// }

// voltarLeaboard.addEventListener('click', function() {
...
function spawnBoss() {
    ...
    function perder() {
        ...
        gameOver.textContent = "Parabéns!!";

        setTimeout(function(){
            game.appendChild(addPlacar);
...
function enviaNome(){
    let nome = document.getElementById("nomedoganhador").value
    criaNome(nome);
    game.removeChild(addPlacar);
    game.appendChild(restart);
}
function fecharNome(){
    game.removeChild(addPlacar);
...
function salvarNome() {
    const liNome = pplacar.querySelectorAll('li'); //pega todos os 'li' dentro de 'tarefa'.
    const listaDeNomes = [];
...
function gerarLixo() {
    if (pontos >= 100) {
        spawnBoss();
        setTimeout(() => {
            alert('Parábens! Você chegou ao final do jogo!' );
        }, 1);
